...
- **src/main/resources/application.properties** - Contains Spring Boot configuration information.  The build pipeline will change the port from the local running port 8080 to the deploy port of 5000 running on AWS

2. Create branch **feature-webapp** and checkout.

3. Modify pom.xml to build your own artifact.  Change the Maven **artifact** and **name** tags to your initials in your Auburn email (replace string "AUEmail").  Maven uses this to build the **war** file name.  For example:
   ```xml
   <name>webapp5a-AUEmail</name>
   
   <artifactId>webapp5a-AUEmail</artifactId>
   ```
   To
   ```xml
...
4. Run the Maven command "**mvn package**" to verify the web application archive (.war file) is successfully generated.  You should see the following line towards the end of the log messamvnges:
...
8. Create an **index.html** file in the **main/resources/templates** directory with the below.  The templates directory is the root directory of the web pages served up by the webserver.
   ...
   <html>
   <body>
   <h1>My Spring Boot Web App AUEmail</h1>
   </body>
   </html>
   ```
- Replace **"AUEmail"** with your initials.
...
21. Validate your pipeline runs successfully and creates the war file package by drilling down into the pipeline stage execution log and searching for "**Building war**"
    ```text
        [INFO] Packaging webapp
        [INFO] Assembling webapp [webapp5a-msb0094] in [/builds/sec-sw-const-sum-24-b/test-group/msb0094/m5-assign-a/target/webapp5a-msb0094-0.0.1-SNAPSHOT]
        [INFO] Processing war project
        [INFO] Building war: /builds/sec-sw-const-sum-24-b/test-group/msb0094/m5-assign-a/target/webapp5a-msb0094-0.0.1-SNAPSHOT.war
        [INFO]
        [INFO] --- spring-boot-maven-plugin:2.7.0:repackage (repackage) @ webapp5a-msb0094 ---
    ```
...
24. Create a stage instance of **deploy** to deploy the artifact to the web application. Modify the snipped below and replace **"AUEmail"** with your initials. In the **aws-eld-deploy** job you can see the **AU_ID** variable is replaced to pick you specific build files.
    ```yaml
    variables:
      MAVEN_OPTS: "-Dmaven.repo.local=$CI_PROJECT_DIR/.m2/repository"
      AU_ID: "azf0047"    
    ...
    aws-elb-deploy:
      # Use gitlab image with AWS CLI already installed
      image: registry.gitlab.com/gitlab-org/cloud-deploy/aws-base:latest
      stage: deploy
      script:
        # Update war file to AWS S3 for staging deployment into Elastic Beanstalk web application server
        - aws s3 cp target/webapp5a-${AU_ID}-0.0.1-SNAPSHOT.war s3://m5a-bucket/webapp5a-${AU_ID}-0.0.1-SNAPSHOT.war
        # Update Application version in Elastic Beanstalk with file from S3
        - aws elasticbeanstalk create-application-version --region us-east-2 --application-name ${AU_ID}-webapp --version-label "${AU_ID}-$CI_COMMIT_SHORT_SHA" --source-bundle S3Bucket="m5a-bucket",S3Key="webapp5a-${AU_ID}-0.0.1-SNAPSHOT.war"
        # Reload/deploy Elastic Beanstalk environment with the new version 
        - aws elasticbeanstalk update-environment --region us-east-2 --environment-name ${AU_ID}-env --version-label "${AU_ID}-$CI_COMMIT_SHORT_SHA"
      dependencies:
        - build
      only:
        - main
      ```
25. We also need to change the HTTP port from our local 8080 to Elastic Beanstalk 5000.  Pipelines can be used to make configuration changes for deployment environments.  Add the following to the **variables** section you pipeline file:
    ```yaml 
    EBS_PORT: 5000
    ```  

    - Add the following lines to the beginning of the **build** job script under the **script** YAML tag and indent correctly.  Before building the war file the script changes the port to match the AWS Elasticbeanstalk port.  The same is also done for the sitemap.xml.
    ```yaml
        # Replace local port 8080 to AWS Elastic Beanstalk 5000
        - sed -i "s|8080|$EBS_PORT|g" src/main/resources/application.properties
        # Change sitemap urls to specific domains for this website.
        - sed -i "s|XXX|${AU_ID}|g" src/main/resources/static/sitemap.xml
...
26. Add, Commit, Push, and Merge to remote repository.

27. Validate your pipeline runs successfully.  If the pipeline fails, inspect the pipeline stage log for error message, check all values in **.gitlab-ci.yml**.

28. Browse to the following url and your web application index.html page should be returned!
- https://AUEmail-app.au-csse-cpsc4970.com/
Note: replace ***AUEmail*** with your initials.

29. Congrats! You have successfully deployed your webapp on AWS!
...
30. Create branch 'pipeline-dast' and checkout.

31. Add a **dast** stage to your pipeline


32. We need to specify the URL to scan and a few other parameters.  This are environment variables added to the **variables** section of the pipeline file.  Replace the ***AUEmail*** with your initials.
    ```yaml
      DAST_WEBSITE: "http://AUEmail-app.au-csse-cpsc4970.com/"
      DAST_BROWSER_SCAN: "true" # use the browser-based GitLab DAST crawler
      DAST_CRAWL_TIMEOUT: "2m"
      DAST_PATHS: "pii.html,internal.html,login,error"
    ```
33. To add DAST scanning by ZAP we just need to add the following lines in the pipeline config file:
    ```yaml
    include:
    ...
    dast:
      dependencies: []
    ```
34. Add, Commit, Push, and Merge to remote repository.

35. Validate your pipeline runs successfully.  If the pipeline fails, inspect the stage failure and check the DAST_WEBSITE setting.

36. In your git lab 5a project you can review the **Security and Compliance-->Vulnerability Report** to see the results of the DAST scanning.  You should have a PII disclosure and Application error disclosure 

# Resolve Vulnerabilities

37. Create branch **resolve-vulnerabilities**

38. Remove or mask the sensitive information from specific html page.

39. Remove the application error content from the html page

40. Commit, push, merge and check the errors are resolved on rescan (right side activity) column

41. Remove the "Still Detected" filter.  Check the ones that were resolved and set the status of these issues to "Resolve" and selecting "Change Status" button.
